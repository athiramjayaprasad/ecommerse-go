...
func AddAddress() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		point_cursor, err := UserCollection.Aggregate(ctx, mongo.Pipeline{match_filter, unwind,group})
		if err != nil {
			c.IndentedJSON(500, "Internal server error")
		}
...
func EditHomeAddress() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		usr_id, err := primitive.ObjectIDFromHex(user_id)
		if err != nil {
			c.IndentedJSON(500, "Internal server error")
		}
		...
		defer cancel()
		filter := bson.D{primitive.E{Key: "_id", Value: usr_id}}  
